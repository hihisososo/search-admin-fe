...
import { UserDictionaryHeader } from "../user/components/UserDictionaryHeader"
...
export default function SynonymDictionary() {
    ...
    const [items, setItems] = useState<SynonymDictionaryItem[]>([])
    ...
    const fetchItems = async () => {
        ...
        try {
            ...
            const response = await apiFetch<PageResponse<SynonymDictionaryItem>>(`/api/v1/dictionaries/synonym?${params}`)
    ...
    const fetchVersions = async () => {
        try {
            ...
            const response = await apiFetch<PageResponse<DeploymentVersion>>(`/api/v1/dictionaries/synonym/versions?${params}`)
    ...
    const handleSaveNew = async () => {
        ...
        try {
            const response = await apiFetch<SynonymDictionaryItem>("/api/v1/dictionaries/synonym", {
                method: "POST",
    ...
    const handleEdit = (item: SynonymDictionaryItem) => {
    ...
    const handleSaveEdit = async (item: SynonymDictionaryItem) => {
        ...
        try {
            const response = await apiFetch<SynonymDictionaryItem>(`/api/v1/dictionaries/synonym/${item.id}`, {
                method: "PUT",
    ...
    const handleCancelEdit = (item: SynonymDictionaryItem) => {
        setItems(prev => prev.map(i => 
            i.id === item.id 
                ? { ...i, isEditing: false }
                : i
    ...
    const handleDeploy = async () => {
        ...
        setDeploying(true)
        try {
            const response = await apiFetch<DeploymentVersion>("/api/v1/dictionaries/synonym/versions", { 
                method: 'POST'
            })
    ...
    return (
        <div className="p-6 space-y-6">
            <Card className="w-full">
                <CardHeader>
                    <CardTitle>유의어사전</CardTitle>
                    <CardDescription>
                        검색 시 같은 의미로 취급될 단어들을 관리할 수 있습니다. "기본키워드, 유의어1, 유의어2" 형태로 등록하여 서로 대체하여 검색됩니다.
                    </CardDescription>
                </CardHeader>
            </Card>
